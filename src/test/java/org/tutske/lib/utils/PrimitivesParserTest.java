...
class PrimitivesParserTest {
	...
	@Test
	void it_should_propagate_errors_from_uri_parsing () {
	...
	@Test
	void it_should_complain_about_non_parsable_dates () {
		assertThrows (RuntimeException.class, () -> {
	...
	@Test
	void it_should_comaplain_when_different_types_cannot_be_converted_into_each_other () {
	...
	@Test
	void it_should_complain_if_it_does_not_know_how_to_convert_into_a_type () {
		assertThrows (IllegalArgumentException.class, () -> {
	...
	@Test
	void it_should_convert_a_type_into_itself () {
		TestType value = new TestType ();
		assertThat (parse (value, TestType.class), is (value));
	}

	@Test
	void it_should_convert_a_type_and_an_assignable_class_to_itself () {
		TestType value = new TestType ();
		assertThat (parse (value, ITest.class), is (value));
	}

	@Test
	void it_should_do_normal_string_parsing () {
		Object value = "true";
		assertThat (parse (value, Boolean.class), is (true));
	}

	@Test
	void it_should_convert_anything_to_an_object () {
		TestType value = new TestType ();
		assertThat (parse (value, Object.class), is (value));
	}

	@Test
	void it_should_be_able_to_convert_anything_into_a_string () {
		TestType value = new TestType () {
	...
	@Test
	void it_should_always_find_a_parser_to_strings () {
	...
	@Test
	void it_should_complain_when_it_does_not_know_how_to_convert_into_a_value () {
		assertThrows (IllegalArgumentException.class, () -> {
	...
	@Test
	void it_should_convert_strings_into_a_path () {
		assertThat (PrimitivesParser.parse ("/tmp/something.txt", Path.class), is (Paths.get ("/tmp/something.txt")));
	}

	@Test
	void it_should_convert_strings_with_tilda_into_a_user_relative_path () {
		assertThat (
